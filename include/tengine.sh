...
Install_Tengine() {
  ...
  if [[ $Apache_version =~ ^[1-2]$ ]]; then
    ...
    [ "$PHP_yn" == 'y' ] && [ -z "`grep '/php${MPHP_version}-fpm_status' $tengine_install_dir/conf/nginx.conf`" ] &&  sed -i "s@index index.html index.php;@index index.html index.php;\n    location ~ /php${MPHP_version}-fpm_status {\n        #fastcgi_pass remote_php_ip:90${MPHP_version};\n        fastcgi_pass unix:/dev/shm/php${MPHP_version}-cgi.sock;\n        fastcgi_index index.php;\n        include fastcgi.conf;\n        allow 127.0.0.1;\n        deny all;\n        }@" $tengine_install_dir/conf/nginx.conf
  fi
...
EOF
  sed -i "s@/home/web/default@$wwwroot_dir/default@" $tengine_install_dir/conf/nginx.conf
