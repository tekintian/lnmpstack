      ...
      while :; do echo
        ...
        if [[ ! $PHP_version =~ ^[1-6]$ ]]; then
          echo "${CWARNING}input error! Please only input number 1,2,3,4,5,6${CEND}"
        	...
        	#change php_install_dir by $PHP_version
			case "${PHP_version}" in
			  1)
			    sed -i "s@^php_install_dir.*@php_install_dir=/usr/local/php-53@" ./options.conf
			    MPHP_version=53
			    ;;
			  2)
			    sed -i "s@^php_install_dir.*@php_install_dir=/usr/local/php-54@" ./options.conf
			    MPHP_version=54
			    ;;
			  3)
			   sed -i "s@^php_install_dir.*@php_install_dir=/usr/local/php-55@" ./options.conf
			   MPHP_version=55
			    ;;
			  4)
			   sed -i "s@^php_install_dir.*@php_install_dir=/usr/local/php-56@" ./options.conf
			   MPHP_version=56
			    ;;
			  5)
			   ...
			   MPHP_version=70
			    ;;
			  6)
			    sed -i "s@^php_install_dir.*@php_install_dir=/usr/local/php-71@" ./options.conf
			    MPHP_version=71
			    ;;
			esac
			#change php_install_dir end
