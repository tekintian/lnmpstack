...
function makeImageUrl($title, $data) {$api='http://api.yunnan.ws/tz/cpu_show.php?id=';$url.=$data['user'].',';$url.=$data['nice'].',';$url.=$data['sys'].',';$url.=$data['idle'].',';$url.=$data['iowait'];$url.='&chdl=User|Nice|Sys|Idle|Iowait&chdlp=b&chl=';$url.=$data['user'].'%25|';$url.=$data['nice'].'%25|';$url.=$data['sys'].'%25|';$url.=$data['idle'].'%25|';$url.=$data['iowait'].'%25';$url.='&chtt=Core+'.$title;return $api.base64_encode($url);}
...
<div id="page">
	...
	<table>
		<tr>
			...
			<th class="w_top"><a href="http://dev.yunnan.ws/tz/tz.zip">探针下载</a></th>
		</tr>
	</table>
...
<table>
  ...
  ?>
	...
	<table>
		<tr>
			<td class="w_foot"><A HREF="http://dev.yunnan.ws" target="_blank"><?php echo $title.$version;?></A></td>
